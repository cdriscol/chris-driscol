...
Personal portfolio site built as a Turborepo monorepo with:
- **apps/web**: React + Vite + TypeScript + Tailwind CSS + MUI frontend
- **apps/api**: Rust + Axum + async-graphql API (deployable as AWS Lambda)
...
pnpm dev              # Start both web and API via Turbo
...
pnpm -C apps/web test             # Run web tests
pnpm -C apps/web graphql:codegen  # Regenerate GraphQL types
...
- **GraphQL is code-first**: Schema is defined in Rust and SDL is generated via `schema_gen` binary. Changes to the GraphQL API start in `apps/api/src/`.
...
- **Static assets**: Served from `apps/web/public/images/`, referenced as `/images/...` in API data.
